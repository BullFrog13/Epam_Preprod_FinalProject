...
using Courses.Models.StudentModels;
...
namespace Courses.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public ManageController(ICourseService course, IJournalService journal)
        {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            {
                List<JournalDto> journalsDto = _journalService.GetJournals().Where(u => u.StudentId.Equals(User.Identity.GetUserId())).ToList();
                List<CourseDto> coursesDto = _courseService.GetCourses();
                List<CourseMarkStudentViewModel> res = new List<CourseMarkStudentViewModel>();
                ...
                foreach (JournalDto journal in journalsDto)
                {
                    foreach(CourseDto courseDto in coursesDto)
                    {
                        if(journal.CourseId == courseDto.Id)
                        {
                            CourseMarkStudentViewModel course = new CourseMarkStudentViewModel();
                            course.Course = mapper.Map<CourseDto, CourseViewModel>(courseDto);
                            course.Mark = journal.Mark;
                            res.Add(course);
                            break;
                        }
                    }
            ...
            {
                List<CourseDto> coursesDto = _courseService.GetCourses().Where(c => c.TutorId.Equals(User.Identity.GetUserId())).ToList();
                List<CourseMarkStudentViewModel> courses = new List<CourseMarkStudentViewModel>();
                var config = new MapperConfiguration(cfg =>
                {
                    cfg.CreateMap<SpecializationDto, SpecializationViewModel>();
                    cfg.CreateMap<CourseDto, CourseViewModel>();
                ...
                {
                    CourseMarkStudentViewModel help = new CourseMarkStudentViewModel();
                    help.Course = mapper.Map<CourseDto, CourseViewModel>(course);
                    courses.Add(help);
                }
        ...
        public async Task<ActionResult> UploadPhoto(HttpPostedFileBase file)
        {
            if(file != null && file.ContentLength > 0)
            {
                ...
                if(fileExt.ToLower().EndsWith(".png") || fileExt.ToLower().EndsWith(".jpg") || fileExt.ToLower()
                    .EndsWith(".gif"))
                {
